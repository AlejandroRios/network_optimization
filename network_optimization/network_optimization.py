...
def dict_to_list(computation_mode, airports_keys, info_acft):
    ''' This function applies a manipulation in the distance, demands and doc matrix to transformate
    them to lists. The process simplifies the LP problem set. The list must follow the correct order
    to apply the opperations starting by the upper triangular matrix'''
    ...
    distances1= np.zeros((len(airports_keys),len(airports_keys)))
    while idx<list_size/2:
        for i in range(len(airports_keys)):
            for j in range(len(airports_keys)):
                if j>i:
                    distances1[i][j] = distances_list[idx]
                    idx = idx+1
    while idx<list_size:
        for i in range(len(airports_keys)):
            for j in range(len(airports_keys)):
    ...
    idx = 0
    list_size = len(airports_keys)**2 - len(airports_keys)
    doc1= np.zeros((len(airports_keys),len(airports_keys)))

    while idx<list_size/2:
        for i in range(len(airports_keys)):
            for j in range(len(airports_keys)):
                if j>i:
                    doc1[i][j] =docs_list[idx]
                    idx = idx+1
    while idx<list_size:
        for i in range(len(airports_keys)):
            for j in range(len(airports_keys)):
...
def flatten_dict(dd, separator ='_', prefix =''):
    '''This function tranform a matrix like dictionary into a flat dictionary'''
...
def restructure_data(aux_mat,n):
    ''' This function restructure the data shape'''
...
def network_optimization(computation_mode, airports_keys, acft):
    ''' Main network optimization function'''
    log.info('==== Start network optimization module ====')

    # Aircraft data is imported from a dictionary using differen data structures, dict_to_list function converts the 
    # needed data into list
    distances_list, demand_list, demand_sum, docs_list, froms_list, tos_list = dict_to_list(computation_mode, airports_keys, acft)

    
    vehicle= acft['vehicle']
    demands = acft['demands']
    doc0 = acft['DOC_ik']
    distances= acft['distances']

    aircraft = vehicle['aircraft']
    operations = vehicle['operations']
    results = vehicle['results']

    pax_capacity =np.floor(aircraft['passenger_capacity'])  # Passenger capacity
    ...
    avg_grnd_time = 0.5
    time_allowed = 13 # Not being used yet
    allowed_planes = [round(13/((distances_list[i]/avg_vel[i])+avg_grnd_time)) for i in range(len(arcs))] # Not being used yet
    ...
    if (computation_mode == 0):
        ...
        flow = LpVariable.dicts("flow",(arcs),0,None,LpInteger) # Passenger flow variable [Int]
        aircrafts = LpVariable.dicts("aircrafts",(arcs),0,10,LpInteger) # Aircraft frequencies variable [Int]

        prob += lpSum([aircrafts[i]*docs_list[i] for i in range(len(arcs))]) # Objective function
        ...
        prob += lpSum([flow[i] for i in range(len(arcs))]) == demand_sum # Constraint total flow == total demand

        for i in range(0,len(nodes)//2):
                prob += lpSum(flow[j] for j in range(0,len(arcs)//2) if tos_list[j] == i) - lpSum(flow[j] for j in range(0,len(arcs)//2) if froms_list[j] == i) == sup_dem_ij[i] # Capacity constraint

        for i in range(0,len(nodes)//2):
                prob += lpSum(flow[j] for j in range(len(arcs)//2,len(arcs)) if tos_list[j] == i) - lpSum(flow[j] for j in range(len(arcs)//2,len(arcs)) if froms_list[j] == i) == sup_dem_ji[i] # Capacity constraint
            
        for i in arcs:
            prob += flow[i] <= aircrafts[i]*avg_capacity # Capacity constraint

        # =============================================================================
        # Solve linear programming problem (Network optimization)
        # =============================================================================
        log.info('==== Start PuLP optimization ====')
        prob.solve(GLPK_CMD(timeLimit=60*5, msg = 0))
        ...
        aircrafts1 = [demand_list[i]/pax_number for i in range(len(arcs))]

        list_of_pax = [aircrafts1[i]*pax_number for i in range(len(arcs))]
    ...
    # =============================================================================
    # Post-processing network results
    # =============================================================================
    min_capacity = 0.5
    ...
    while idx<list_size/2:
        for i in range(len(airports_keys)):
            for j in range(len(airports_keys)):
                if j>i:
                    aircrafts1[i][j] = list_aircraft[idx]
                    idx = idx+1
    while idx<list_size:
        for i in range(len(airports_keys)):
            for j in range(len(airports_keys)):
    ...
    for i in range(len(fraction_1_list)):
        if (list_of_pax[i] <= 0 or fraction_1_list[i] <= 0):
            ...
            revenue_1_list.append(avg_capacity*fraction_1_list[i]*distances_list[i]*(avg_capacity*fraction_1_list[i]*average_ticket_price)/(avg_capacity*fraction_1_list[i]*distances_list[i]))

    revenue_1_list = [0 if x != x else x for x in revenue_1_list]

    revenue_2_list = []
    for i in range(len(fraction_2_list)):
        if (list_of_pax[i] <= 0 or fraction_2_list[i] <= min_capacity):
            revenue_2_list.append(0)
        else:
            revenue_2_list.append(avg_capacity*fraction_2_list[i]*distances_list[i]*(avg_capacity*fraction_2_list[i]*average_ticket_price)/(avg_capacity*fraction_2_list[i]*distances_list[i]))
    revenue_2_list = [0 if x != x else x for x in revenue_2_list]

    revenue_tot_list = [x + y for x, y in zip(revenue_1_list,revenue_2_list)]
    revenue_tot = sum(revenue_tot_list)
    ...
    while idx<list_size/2:
        for i in range(len(airports_keys)):
            for j in range(len(airports_keys)):
                if j>i:
                    revenue_mat[i][j] = revenue_tot_list[idx]
                    idx = idx+1
    while idx<list_size:
        for i in range(len(airports_keys)):
            for j in range(len(airports_keys)):
    ...
    for i in range(len(airports_keys)):
        for j in range(len(airports_keys)):
            DOC_proccessed[i][j] = DOCmat[i][j]*aircrafts1[i][j]
    ...
    list_aircraft_db = pd.DataFrame(list_of_airplanes_processed)
    list_aircraft_db.to_csv('Database/Network/frequencies.csv')
