...
def passenger_load(list_flow, airports_number, inputs):
    ...
    capacity_matrix = list_to_matrix(capacity,airports_number)
    fraction = np.nan_to_num(flow_matrix/capacity_matrix)
    full_acft_matrix = np.floor(fraction)
    frac_acft_matrix = fraction-full_acft_matrix

    full_acft_list = matrix_to_list(full_acft_matrix, airports_number)
    frac_acft_list = matrix_to_list(frac_acft_matrix, airports_number)


    print(full_acft_matrix)

    print(frac_acft_matrix)
    
    aux = np.zeros((airports_number,airports_number))
    for i in range(airports_number):
        for j in range(airports_number):
            if frac_acft_matrix[i][j] > 0.5:
                aux[i][j] = 1
                
    total_acft_matrix = full_acft_matrix  + aux

    print(total_acft_matrix)

#     print('full acft:', full_acft)
#     print('frac acft:', frac_acft)
